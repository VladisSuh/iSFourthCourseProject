...
import (
	...
	"iSFourthCourseProject/proto/chatpb"
...
func SendInvitationHandler(c *gin.Context) {
	...
	// Отправить уведомление через WebSocket уведомлений
...
func RespondInvitationHandler(c *gin.Context) {
	...
	if action == "accepted" {
		...
		// Сначала вызываем JoinRoom для второго участника
		...
		privateKey, err := algoprotoc.GeneratePrivateKey(prime)
		if err != nil {
		...
		redirectURL := "/chats/chat?room_id=" + roomID
		...
		c.JSON(http.StatusOK, gin.H{"message": "Приглашение отклонено", "redirect_url": "/chats/menu"})
	} else {
