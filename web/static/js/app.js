...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteChatButton = document.getElementById('delete-chat-btn'); // Новая кнопка
    ...
    if (deleteChatButton) {
        deleteChatButton.addEventListener('click', async () => {
            if (confirm('Вы уверены, что хотите удалить эту комнату? Это действие нельзя отменить.')) {
                ...
                try {
                    const response = await fetch(`/chats/chat?room_id=${encodeURIComponent(roomId)}`, {
                        method: 'DELETE',
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        window.location.href = 'chats/menu';
                    } else {
    ...
    if (fileUploadForm) {
        fileUploadForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`/chats/chat/send-file?room_id=${encodeURIComponent(roomID)}`, {
                    method: 'POST',
                    body: formData,
                });
    ...
    async function fetchFiles() {
        try {
            const response = await fetch(`/chats/chat/files?room_id=${encodeURIComponent(roomID)}`);
            if (response.ok) {
                ...
                files.forEach(file => {
                    ...
                    fileItem.innerHTML = `
                        <a href="/chats/chat/download-file?room_id=${encodeURIComponent(roomID)}&file_name=${encodeURIComponent(file.file_name)}" target="_blank">${escapeHtml(file.file_name)}</a>
    ...
    function connectWebSocket(roomID) {
        ...
        socket.onmessage = function(event) {
            ...
            if (data.type === "chat") {
                ...
                appendMessage("Система", data.notification); // пример
            }
        };
